...
public class DriverRepositoryTest : IDisposable {
	...
	public async Task GetByIdAsync_WhenDriverExists_ShouldReturnDriver() {
		...
		// Act
		Driver? result = await _repository.GetByIdAsync(driver.Id);
	...
	public async Task GetByIdAsync_WhenDriverDoesNotExist_ShouldReturnNull() {
		...
		// Act
		Driver? result = await _repository.GetByIdAsync(nonExistingId);
	...
	public async Task GetAllAsync_WhenDriversExist_ShouldReturnAllDrivers() {
		// Arrange
		Driver driver1 = CreateValidDriver("Luis RodrÃ­guez");
		Driver driver2 = CreateValidDriver("Ana MartÃ­nez");
		Driver driver3 = CreateValidDriver("Carlos GÃ³mez");

		await _dbContext.Drivers.AddRangeAsync(driver1, driver2, driver3);
		await _dbContext.SaveChangesAsync();

		// Act
		IReadOnlyList<Driver> result = await _repository.GetAllAsync();
	...
	public async Task AddAsync_ShouldAddDriverToDatabase() {
		...
		// Assert
		Driver? savedDriver = await _dbContext.Drivers.FindAsync(driver.Id);
	...
	public async Task Update_ShouldUpdateDriverInDatabase() {
		...
		// Act
		Driver? driverToUpdate = await _dbContext.Drivers.FindAsync(driver.Id);
		...
		// Assert
		Driver? updatedDriver = await _dbContext.Drivers.FindAsync(driver.Id);
	...
	public async Task Remove_ShouldDeleteDriverFromDatabase() {
		...
		// Assert
		Driver? deletedDriver = await _dbContext.Drivers.FindAsync(driver.Id);

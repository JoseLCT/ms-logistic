...
public class CustomerRepositoryTest : IDisposable {
	...
	public async Task GetAllAsync_WhenCustomersExist_ShouldReturnAllCustomers() {
		// Arrange
		Customer customer1 = CreateValidCustomer("Mike Brown");
		Customer customer2 = CreateValidCustomer("Jane Smith");
		Customer customer3 = CreateValidCustomer("Bob Johnson");
	...
	public async Task AddAsync_ShouldAddCustomerToDatabase() {
		...
		// Assert
		Customer? savedCustomer = await _dbContext.Customers.FindAsync(customer.Id);
	...
	public async Task Update_ShouldUpdateCustomerInDatabase() {
		...
		// Act
		Customer? customerToUpdate = await _dbContext.Customers.FindAsync(customer.Id);
		customerToUpdate!.SetFullName("Juan Perez");
		_repository.Update(customerToUpdate);
		await _dbContext.SaveChangesAsync();

		// Assert
		Customer? updatedCustomer = await _dbContext.Customers.FindAsync(customer.Id);
	...
	public async Task Remove_ShouldDeleteCustomerFromDatabase() {
		...
		// Assert
		Customer? deletedCustomer = await _dbContext.Customers.FindAsync(customer.Id);

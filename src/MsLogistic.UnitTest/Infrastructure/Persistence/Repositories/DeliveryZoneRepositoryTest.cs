...
public class DeliveryZoneRepositoryTest : IDisposable {
	...
	public async Task GetByIdAsync_WhenDeliveryZoneExists_ShouldReturnDeliveryZone() {
		// Arrange
		DeliveryZone zone = CreateValidDeliveryZone();
		await _dbContext.DeliveryZones.AddAsync(zone);
		await _dbContext.SaveChangesAsync();

		// Act
		DeliveryZone? result = await _repository.GetByIdAsync(zone.Id);
	...
	public async Task GetByIdAsync_WhenDeliveryZoneDoesNotExist_ShouldReturnNull() {
		...
		// Act
		DeliveryZone? result = await _repository.GetByIdAsync(nonExistingId);
	...
	public async Task GetAllAsync_WhenDeliveryZonesExist_ShouldReturnAllDeliveryZones() {
		// Arrange
		DeliveryZone zone1 = CreateValidDeliveryZone(code: "ABC-123", name: "North Zone");
		DeliveryZone zone2 = CreateValidDeliveryZone(code: "XYZ-456", name: "South Zone");
		DeliveryZone zone3 = CreateValidDeliveryZone(code: "DEF-789", name: "East Zone");

		await _dbContext.DeliveryZones.AddRangeAsync(zone1, zone2, zone3);
		await _dbContext.SaveChangesAsync();

		// Act
		IReadOnlyList<DeliveryZone> result = await _repository.GetAllAsync();
	...
	public async Task AddAsync_ShouldAddDeliveryZoneToDatabase() {
		...
		// Assert
		DeliveryZone? savedZone = await _dbContext.DeliveryZones.FindAsync(zone.Id);
	...
	public async Task Update_ShouldUpdateDeliveryZoneInDatabase() {
		...
		// Act
		DeliveryZone? zoneToUpdate = await _dbContext.DeliveryZones.FindAsync(zone.Id);
		zoneToUpdate!.SetName("Updated Zone");
		_repository.Update(zoneToUpdate);
		await _dbContext.SaveChangesAsync();

		// Assert
		DeliveryZone? updatedZone = await _dbContext.DeliveryZones.FindAsync(zone.Id);
	...
	public async Task Remove_ShouldDeleteDeliveryZoneFromDatabase() {
		...
		// Assert
		DeliveryZone? deletedZone = await _dbContext.DeliveryZones.FindAsync(zone.Id);

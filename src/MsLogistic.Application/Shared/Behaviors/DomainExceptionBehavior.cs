...
public class DomainExceptionBehavior<TRequest, TResponse>
	...
	private static Func<Error, TResponse> CreateFailureFactory() {
		...
		if (responseType.IsGenericType && responseType.GetGenericTypeDefinition() == typeof(Result<>)) {
			...
			MethodCallExpression callExpression = Expression.Call(failureMethod, errorParam);
			UnaryExpression convertExpression = Expression.Convert(callExpression, typeof(TResponse));
